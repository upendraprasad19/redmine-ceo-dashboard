...
export default function ForgotPasswordPage() {
  ...
  const btn = (enabled) => ({
    width: '100%',
    background: enabled ? C.blue : 'rgba(26,110,245,0.35)',
    border: 'none',
    borderRadius: 10,
    padding: 13,
    fontSize: 13,
    ...
    <div style={pageWrap}>
      <div style={{ width: '100%', maxWidth: 420 }}>
        ...
        <div style={cardStyle}>
          {step === 1 && (
            <form onSubmit={fetchChannels}>
              ...
              <div style={{ marginTop: 16 }}>
                <button type="submit" style={btn(!!username && !busy)} disabled={!username || busy}>
            ...
            <form onSubmit={sendCode}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 18 }}>
                <ChannelBox
                  ...
                  hint={
                    channels.telegramAvailable ? 'Linked account' : 'Not linked on this account'
                  }
                />
                <ChannelBox
                  label="Email"
              ...
              <button
                type="submit"
                style={btn(picked.length > 0 && !busy)}
          ...
          {step === 3 && (
            <form onSubmit={verifyCode}>
              ...
              <div style={{ fontSize: 12, color: C.dim, marginBottom: 18 }}>
                Enter the 6-digit code we sent (expires in 15 min).
              </div>
              <input
                style={{
                  ...input,
          ...
          {step === 4 && (
            <form onSubmit={submitNew}>
              ...
              <div style={{ marginTop: 16 }}>
                <button
                  type="submit"
                  style={btn(!!newPw && !!newPw2 && !busy)}
                  disabled={!newPw || !newPw2 || busy}
                >
          ...
          {step === 'done' && (
            <div style={{ textAlign: 'center', padding: '10px 0' }}>
              ...
              <div style={{ fontSize: 14, color: C.green, fontWeight: 600, marginBottom: 6 }}>
                Password updated
              </div>
          ...
          {step === 'unavailable' && (
            <div style={{ padding: '10px 0' }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: C.amber, marginBottom: 8 }}>
                No reset channels available
              </div>
              <div style={{ fontSize: 12, color: C.dim, lineHeight: 1.5 }}>
                Neither Telegram nor an email address is on file for this username. Contact your
                admin to reset your password manually.
              </div>
              <div style={{ marginTop: 18 }}>
                <button onClick={() => router.push('/login')} style={btn(true)}>
                  Back to sign-in
                </button>
              </div>
            </div>
        ...
        <div style={{ textAlign: 'center', marginTop: 20 }}>
          <a href="/login" style={{ fontSize: 11, color: C.dim, textDecoration: 'none' }}>
            ← Back to sign-in
          </a>
        </div>
      </div>
    ...
    >
      <div>
        ...
        <div
          style={{
            fontSize: 10,
            color: disabled ? 'rgba(240,244,255,0.22)' : 'rgba(240,244,255,0.45)',
            fontWeight: 400,
            marginTop: 2,
          }}
        >
      ...
      <div style={{ fontSize: 16, color: selected && !disabled ? '#1A6EF5' : 'transparent' }}>
        ✓
      </div>
    </button>
  )
