...
export default async function handler(req, res) {
  ...
  if (briefing_days !== undefined && !VALID_BRIEFING_DAYS.includes(briefing_days)) {
    return res.status(400).json({ error: `briefing_days must be one of: ${VALID_BRIEFING_DAYS.join(', ')}` });
  }
  if (morning_briefing !== undefined && !VALID_MORNING_BRIEFING.includes(morning_briefing)) {
    return res.status(400).json({ error: `morning_briefing must be one of: ${VALID_MORNING_BRIEFING.join(', ')}` });
  }
  if (notification_channels !== undefined) {
    if (!Array.isArray(notification_channels) || !notification_channels.every(c => VALID_CHANNELS.includes(c))) {
      return res.status(400).json({ error: `notification_channels must be an array of: ${VALID_CHANNELS.join(', ')}` });
    }
  }
