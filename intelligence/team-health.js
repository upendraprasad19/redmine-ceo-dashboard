      ...
      try {
        ...
        const deliveryRows = await sql`
          ...
          WHERE assigned_to_id = ANY(${memberIds})
            AND closed_at >= ${weekStartStr}::date
        `;
        const dr = (deliveryRows && deliveryRows[0]) || {};
        ...
        if (hoursRows && hoursRows.length > 1) {
          ...
          const variance =
            dailyHours.reduce((sum, val) => sum + Math.pow(val - mean, 2), 0) / dailyHours.length;
        ...
        `;
        const avgBlockerHours =
          blockerRows && blockerRows[0] && blockerRows[0].avg_blocker_hours
        ...
        `;
        const rr = (reopenRows && reopenRows[0]) || {};
