...
async function calculateVelocity(teamOrUserId, periodWeeks = 4) {
  ...
  try {
    ...
    const firstAvg =
      firstHalf.length > 0
        ? firstHalf.reduce((s, w) => s + w.closed, 0) / firstHalf.length
        : 0;
    const secondAvg =
      secondHalf.length > 0
        ? secondHalf.reduce((s, w) => s + w.closed, 0) / secondHalf.length
        : 0;
...
async function predictDeadlineMiss(projectId) {
  ...
  try {
    ...
    const teamRows = await sql`
      ...
      LIMIT 1
    `;
    const teamName = teamRows && teamRows[0] ? teamRows[0].team : null;
    ...
    `;
    const closed4w = (velRows && velRows[0] && velRows[0].closed_4w) || 0;
