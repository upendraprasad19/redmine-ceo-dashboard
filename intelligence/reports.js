...
async function buildWeeklyReport(team, periodDays = 7) {
  ...
  try {
    ...
    `
    const completed = completedRows?.[0] || { completed: 0, on_time: 0 }
    ...
    `
    const hours = hoursRows?.[0] || { total_hours: 0, users_logging: 0 }
    ...
    `
    const overdue = overdueRows?.[0] || { overdue_count: 0, overdue_list: null }
    ...
    const blockerRows = await sql`
      ...
      WHERE assigned_to_id = ANY(${memberIds})
        AND status = 'Blocked'
    `
    const blockers = blockerRows?.[0] || { blocked_count: 0, blocked_list: null }
    ...
    `
    const newTickets = newRows?.[0]?.new_tickets || 0
    ...
    const response = await chat([
      {
        role: 'system',
        content:
          'You are a professional project management report writer. Be concise, data-driven, and actionable.',
      },
