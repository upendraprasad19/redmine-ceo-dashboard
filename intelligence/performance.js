...
async function calculatePerformanceScores(period = 'daily') {
  ...
  try {
    ...
    `;
    const avgClosed = (avgRows && avgRows[0] && avgRows[0].avg_closed) || 1;
    const avgResolution = (avgRows && avgRows[0] && avgRows[0].avg_resolution) || 24;
    ...
    for (const user of dashUsers) {
      try {
        ...
        `;
        const hoursLogged = (hoursRows && hoursRows[0] && hoursRows[0].total_hours) || 0;
        ...
        const collabRows = await sql`
          ...
          WHERE ij.author_id = ${uid}
            ...
            AND ij.notes != ''
        `;
        const blockersHelped = (collabRows && collabRows[0] && collabRows[0].blockers_helped) || 0;

        // ── Compute 5 scores (0-100) ────────────────────────────
        // Output: how many tickets closed relative to average
        const outputScore = clamp(Math.round((ticketsClosed / Math.max(avgClosed, 1)) * 60 + (hoursLogged > 0 ? 20 : 0)), 0, 100);
        ...
        const prevRows = await sql`
          ...
          LIMIT 1
        `;
        const prevScore = prevRows && prevRows[0] ? prevRows[0].overall_score : null;
