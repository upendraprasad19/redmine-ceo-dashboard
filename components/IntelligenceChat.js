  ...
  return parts.map((part, i) => {
    ...
    return boldParts.map((bp, j) => {
      if (bp.startsWith("**") && bp.endsWith("**")) {
        return <strong key={`${i}-${j}`} style={{ color: C.white }}>{bp.slice(2, -2)}</strong>;
      }
...
export default function IntelligenceChat({ currentUser }) {
  ...
  async function handleSend() {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: "assistant", content: "Network error. Please check your connection and try again.", created_at: new Date().toISOString() }]);
    } finally {
      setSending(false);
    }
  }
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div>
          <div style={{
            ...
            <svg width="20" height="20" fill="none" stroke={C.blue} strokeWidth="1.8" viewBox="0 0 24 24">
        ...
        <div style={{
          ...
          fontSize: 11, color: C.blueLight, fontWeight: 600,
        }}>
          AI Assistant
        </div>
      </div>
        ...
        {!loadingHistory && messages.length === 0 && (
          <div style={{
            ...
            <div style={{
              ...
              <svg width="28" height="28" fill="none" stroke={C.blue} strokeWidth="1.5" viewBox="0 0 24 24">
            ...
            <div style={{ textAlign: "center" }}>
              ...
              <div style={{ fontSize: 12, color: C.dimmer, lineHeight: 1.6, maxWidth: 320 }}>
                Ask about overdue tickets, team workload, project deadlines, or anyone's status.
        ...
        {messages.map((msg, i) => (
          ...
          >
            <div style={{
              ...
              <div style={{
                fontSize: 9, color: msg.role === "user" ? "rgba(255,255,255,0.5)" : C.dimmer,
      ...
      {showMentions && filteredPeople.length > 0 && (
        <div style={{
          ...
          marginBottom: 4, boxShadow: "0 -4px 20px rgba(0,0,0,0.4)",
        }}>
          {filteredPeople.map(p => (
            ...
            >
              ...
              <div>
                ...
                <div style={{ fontSize: 10, color: C.dimmer }}>{p.team} · {p.role}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{
        display: "flex", alignItems: "center", gap: 10,
        padding: "12px 0", borderTop: `1px solid ${C.border}`, marginTop: 8,
      }}>
        <input
          ref={inputRef}
          type="text"
