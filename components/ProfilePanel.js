...
export default function ProfilePanel({ open, onClose, onLogout }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/profile/snapshot')
      ...
      .then(setData)
      .catch((_e) => setErr('Failed to load profile'))
  }, [open])
          ...
          <>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 11, color: C.dim, marginTop: 2 }}>
                  ...
                  {me.team && (
                    <>
                      {' '}
                      · <span style={{ color: C.blueLight }}>{me.team}</span>
                    </>
                  )}
                </div>
              </div>
            ...
            <div style={{ marginTop: 16 }}>
              ...
              <Row
                ...
                valueColor={me.email ? C.white : C.dim}
              />
              <Row
                label="Telegram"
                  ...
                  <>
                    ...
                    {snap.topRisk?.length > 0 && (
                      <div style={{ marginTop: 12 }}>
                        <div style={{ fontSize: 10, color: C.dimmer, marginBottom: 6 }}>
                          Projects with most overdue
                        </div>
                        {snap.topRisk.map((r) => (
                          <div
                            key={r.name}
            ...
            <Section title="Change password">
              ...
              >
                ...
                />
                {pwMsg && (
                  <div style={{ fontSize: 11, color: pwMsg.type === 'ok' ? C.green : C.red }}>
                    {pwMsg.text}
                  </div>
                )}
                <button
                  type="submit"
                  disabled={!pwOld || !pwNew || !pwNew2}
                  style={{
                    background: C.blue,
                    border: 'none',
                    color: C.white,
...
function Kpi({ label, value, color }) {
  return (
    ...
    >
      <div
        style={{ fontSize: 9, color: C.dimmer, letterSpacing: '0.1em', textTransform: 'uppercase' }}
      >
        {label}
      </div>
      <div
