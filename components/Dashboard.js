...
const _MOCK = { people: [], projects: [], tickets: [], timeLogs: [] }
...
const PROJECT_ORDER = [
  ...
  'Liability (iPLUS 2.0)',
  'iPLUS RW',
  'Rule Engine for Liability (iPLUS2.0)',
  'Widget Based iPACS',
  'iPACS 2.0',
  'iPACS',
  'iPPO 2',
...
const riskColor = (r) =>
  ({ critical: C.red, high: C.amber, medium: C.blueLight, low: C.green })[r?.toLowerCase()] || C.dim
const statusColor = (s) =>
  ({ 'In Progress': C.blueLight, Review: C.green, Blocked: C.red, Todo: C.dimmer })[s] || C.dim
...
const Dot = ({ color, pulse }) => (
  <span
    style={{
      ...
      flexShrink: 0,
      background: color,
      animation: pulse ? 'pulseRing 2s ease infinite' : 'none',
    }}
  />
)
...
function Card({ children, style, onClick }) {
  return (
    <div
      ...
      onMouseEnter={(e) => {
        if (onClick) e.currentTarget.style.borderColor = C.borderHi
      }}
      onMouseLeave={(e) => {
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          border: `1px solid ${ticket.overdue ? `${C.red}66` : C.borderHi}`,
          borderRadius: 20,
          padding: '36px 40px',
          width: 560,
          maxWidth: '90vw',
      ...
      >
        ...
        <div style={{ marginBottom: 20 }}>
          <div style={{ fontSize: 11, color: C.dimmer, letterSpacing: '0.12em', marginBottom: 6 }}>
            {ticket.id}
          </div>
          <div
            style={{
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 24, flexWrap: 'wrap' }}>
          ...
          {ticket.overdue && (
            ...
            >
              <span style={{ fontSize: 11, color: C.red, fontWeight: 700 }}>OVERDUE</span>
            </div>
          )}
        </div>
        <div
        ...
        >
          {[
            ['Assigned To', ticket.to],
            ['Assigned By', ticket.by],
            ['Team', ticket.team],
        ...
        <div style={{ paddingTop: 20 }}>
          ...
          >
            ...
            <div style={{ fontSize: 11, color: C.dimmer }}>
              Updated by{' '}
              <span style={{ color: C.blueLight, fontWeight: 600 }}>{ticket.commentBy}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      >
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 11, color: C.dimmer, marginTop: 2 }}>
            <span style={{ color: C.green }}>{totalLogged} logged</span> ·{' '}
            <span style={{ color }}>{totalNotLogged} not logged</span>
          </div>
        </div>
        <Chevron open={open} />
      </div>
        ...
        <div style={{ padding: '12px 20px 16px', background: C.bg }}>
          {showBars && Object.keys(loggedByTeam || {}).length > 0 && (
            <div style={{ marginBottom: 16 }}>
              ...
              >
                Who Logged Time (by team)
              </div>
              {Object.entries(loggedByTeam)
                .sort(([a], [b]) => a.localeCompare(b))
                .map(([team, members]) => (
                  <div key={team} style={{ marginBottom: 8 }}>
                    <div
                      onClick={() => toggleTeam(`logged-${team}`)}
                      style={{
                        display: 'flex',
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        >
                          {members.reduce((s, m) => s + m.hours, 0).toFixed(1)}h
                        </span>
                          ...
                          .map((m) => (
                            ...
                            >
                              ...
                              >
                                <div
                                  style={{
                                    width: `${Math.min((m.hours / maxHours) * 100, 100)}%`,
                                    height: '100%',
                                    background: C.green,
                                    borderRadius: 99,
                                    transition: 'width .3s',
          ...
          {Object.keys(noLogByTeam || {}).length > 0 && (
            <div>
              ...
              {Object.entries(noLogByTeam)
                .sort(([a], [b]) => a.localeCompare(b))
                .map(([team, members]) => (
                  <div key={team} style={{ marginBottom: 6 }}>
                    <div
                      onClick={() => toggleTeam(`nolog-${team}`)}
                      style={{
                        display: 'flex',
                        justifyContent: 'space-between',
                        alignItems: 'center',
          ...
          {Object.keys(noLogByTeam || {}).length === 0 && (
            <div style={{ color: C.green, padding: 12, textAlign: 'center', fontSize: 12 }}>
...
function NoTimeLogModal({ onClose }) {
  ...
  function exportCSV(period) {
    ...
    a.href = url
    a.download = `timelog-${period}-${new Date().toISOString().split('T')[0]}.csv`
    a.click()
      ...
      >
        ...
        >
          <div>
            ...
            <div style={{ fontSize: 12, color: C.dimmer, marginTop: 4 }}>
              {data ? `${data.total_users} team members tracked` : 'Loading...'}
            </div>
          </div>
        ...
        {data?.team_summary && (
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 16 }}>
            {Object.entries(data.team_summary)
              .sort(([a], [b]) => a.localeCompare(b))
              .map(([team, s]) => (
                <div
                  key={team}
...
function WorkloadByTeam({ workload = [], memberHours = [], compact = false }) {
  ...
  for (const m of memberHours) {
    ...
    membersByTeam[m.team].push({
      id: m.id,
      name: m.name,
      hours: parseFloat(m.hours_today || 0),
      tickets: parseInt(m.open_tickets || 0, 10),
    })
  }
    ...
    return (
      <Card style={{ flex: 1, minWidth: 0, padding: '16px 18px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 12 }}>
          {workload.length === 0 && (
            <div style={{ color: C.dimmer, fontSize: 13 }}>No team data</div>
          )}
          {workload.map((w) => {
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  >
                    {teamHoursTotal.toFixed(1)}h today
                  </span>
                  <span style={{ fontSize: 10, color: C.dim }}>
                    avg {w.avg_tickets_per_person} tickets
                  </span>
                </div>
              </div>
            )
  ...
  return (
    <Card style={{ padding: 0, overflow: 'hidden' }}>
      ...
      <div style={{ padding: '4px 0' }}>
        {workload.length === 0 && (
          <div style={{ padding: 20, color: C.dimmer, fontSize: 13 }}>No team data available</div>
        )}
        {workload.map((w, i) => {
          ...
          return (
            <div key={w.team}>
              <div
                onClick={() => toggleTeam(w.team)}
                style={{ padding: '12px 22px', cursor: 'pointer', transition: 'background .15s' }}
                          ...
                          >
                            <div
                              style={{
                                width: `${Math.min((m.hours / maxHours) * 100, 100)}%`,
                                height: '100%',
                                background: m.hours > 0 ? C.green : `${C.red}44`,
                                borderRadius: 99,
                                transition: 'width .3s',
                              }}
                            />
...
function Overview({ overview = {}, people = [], tickets = [], timeLogs = [], currentUser = null }) {
  ...
  useEffect(() => {
    fetch('/api/pm-pulse/executive-snapshot')
      .then((r) => r.json())
      .then(setExecSnapshot)
      .catch(() => {})
  }, [])
  ...
  return (
    <div
      style={{ display: 'flex', flexDirection: 'column', gap: 24, animation: 'fadeIn .4s ease' }}
    >
      {/* ── PINNED INSIGHTS ── */}
      ...
      {(kpis.overdue_tickets > 0 || kpis.no_time_log > 0) && (
        ...
        >
          ...
          <span style={{ fontSize: 13, color: C.red }}>
            {kpis.overdue_tickets} tickets overdue · {kpis.no_time_log} members haven't logged time
            today
          </span>
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 16 }}>
        <Card>
          ...
          <BigNum
            value={`${(kpis.headcount || 0) - (kpis.on_leave || 0)} / ${kpis.headcount || 0}`}
            color={C.white}
          />
      ...
      <div>
        ...
        >
          Executive Snapshot — Active Tickets
        </div>
...
function _TeamLeave({ people = [], onSelectPerson }) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16 }}>
        {[
          ...
          {
            label: 'Available',
            val: people.length - onLeave,
            color: C.green,
            sub: 'Working today',
          },
          {
            label: 'Teams',
            val: Object.keys(teamGroups).length,
            color: C.white,
            sub: 'Total teams',
          },
        ].map((k) => (
                  ...
                  >
                    ...
                    {m.leave ? (
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        ...
                        <span style={{ fontSize: 11, color: C.amber }}>{m.leave}</span>
                      </div>
                    ) : (
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        <Dot color={C.green} />
                        <span style={{ fontSize: 11, color: C.dim }}>Active</span>
                      </div>
                    )}
                    <svg
                      width="12"
...
function TimeLogs({ timeLogs: initialTimeLogs = [], overview = {} }) {
  ...
  useEffect(() => {
    if (range === 'daily') {
      setLogs(initialTimeLogs)
      return
    }
    if (range === 'custom' && (!customApplied || !customFrom || !customTo)) return
    let cancelled = false
    const qs =
      range === 'custom' ? `range=custom&from=${customFrom}&to=${customTo}` : `range=${range}`
  ...
  }, [range, initialTimeLogs, customApplied, customTo, customFrom])
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
        {['daily', 'weekly', 'monthly', 'quarterly', 'yearly', 'custom'].map((r) => (
          <button
            ...
            onClick={() => setRange(r)}
            style={{
              background: range === r ? C.blue : 'transparent',
              border: `1px solid ${range === r ? C.blue : C.border}`,
              color: range === r ? C.white : C.dim,
              borderRadius: 8,
              padding: '7px 18px',
              fontSize: 11,
              fontWeight: 600,
      ...
      {range === 'custom' && (
        ...
        >
          ...
          />
          <button
            onClick={() => setCustomApplied((v) => v + 1)}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16 }}>
        {[
          ...
          {
            label: 'Yesterday',
            val: `${yesterdayHours}h`,
            color: C.blueLight,
            sub: 'Total logged',
          },
          {
            label: 'On Target',
            val: `${logs.filter((t) => t.logged).length}/${logs.length}`,
            color: C.green,
            sub: 'Logged today',
          },
          {
            label: 'Missing',
            val: missing.length,
            color: missing.length ? C.red : C.green,
            sub: 'Need to log',
          },
        ].map((k) => (
          <Card key={k.label}>
      ...
      {missing.length > 0 && (
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <Dot color={C.red} pulse />
            <span style={{ fontSize: 13, color: C.red }}>
              {missing.length === 1 ? missing[0].name : `${missing.length} members`} haven't logged
              time today
            </span>
          </div>
          <button
        ...
        {Object.entries(teamGroups).map(([team, members]) => {
          ...
          return (
            <TeamAccordion
              key={team}
              teamName={team}
              count={`${teamTotal}h logged`}
              meta={miss > 0 ? `${miss} missing` : null}
            >
              {members.map((m, i) => (
                <div key={m.name}>
                  <div
                  ...
                  >
                    ...
                    <span style={{ width: 120, fontSize: 13, color: C.white, fontWeight: 500 }}>
                      {m.name}
                    </span>
                    {m.logged ? (
                      <>
    ...
    <div style={{ border: `1px solid ${C.border}`, borderRadius: 12, overflow: 'hidden' }}>
      ...
      >
        ...
        <span
          style={{
            marginLeft: 'auto',
            background: rows.length ? `${C.amber}22` : C.border,
            border: `1px solid ${rows.length ? `${C.amber}55` : C.border}`,
            borderRadius: 20,
            padding: '2px 10px',
            fontSize: 11,
            color: rows.length ? C.amber : C.dimmer,
            fontWeight: 600,
          }}
        >
          {rows.length}
      ...
      {open && (
        <div style={{ padding: '0 0 8px' }}>
          {rows.length === 0 ? (
            <div style={{ padding: '16px 20px', color: C.dimmer, fontSize: 12 }}>
              No issues found.
            </div>
          ) : (
...
function Tickets({ tickets = [], onSelectTicket }) {
  ...
  const reviewCount = filteredTix.filter(
  ...
  function sortHeader(label, proj) {
    ...
    return (
      <div
        key={label}
        onClick={colKey && proj ? () => handleSort(colKey, proj) : undefined}
        style={{
          fontSize: 9,
          letterSpacing: '0.1em',
          textTransform: 'uppercase',
  ...
  function anomRow_reopen(r) {
    return [
      ...
      <a
        href={`https://redmine.thinkingcode.com/issues/${r.redmine_id}`}
        target="_blank"
        rel="noreferrer"
        style={{ fontSize: 11, color: C.blueLight, textDecoration: 'none', fontWeight: 600 }}
      >
        #{r.redmine_id}
      </a>,
      <span
  ...
  function anomRow_notime(r) {
    return [
      ...
      <span
        style={{
          fontSize: 11,
          color: r.due_date && new Date(r.due_date) < new Date() ? C.red : C.dim,
        }}
      >
        {fmtDate(r.due_date)}
      </span>,
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      {/* KPI cards */}
      <div
        style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16, maxWidth: 800 }}
      >
        {[
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        {BUCKETS.map((b) => (
          <button
            ...
            onClick={() => setBucket(b)}
            style={{
              background: bucket === b ? C.blue : 'transparent',
              border: `1px solid ${bucket === b ? C.blue : C.border}`,
              color: bucket === b ? C.white : C.dim,
              borderRadius: 20,
              padding: '6px 16px',
              fontSize: 11,
              fontWeight: 600,
          ...
          >
            {b === 'All'
              ? `All (${tickets.length})`
              : `${b} (${tickets.filter((t) => getProjectBucket(t.project_name || '') === b).length})`}
          </button>
        ))}
      </div>
          ...
          .map(([proj, tix]) => {
            ...
            return (
              <TeamAccordion
                key={proj}
                teamName={proj}
                count={`${tix.length} tickets`}
                meta={od > 0 ? `${od} overdue` : null}
              >
                <div style={{ overflowX: 'auto' }}>
                  <div style={{ minWidth: 1100 }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                      {groupTix.map((t, i) => {
                        const rowBg = t.overdue
                          ? `${C.red}12`
                          : t.due_date &&
                              (new Date(t.due_date) - Date.now()) / 86400000 <= 3 &&
                              (new Date(t.due_date) - Date.now()) / 86400000 >= 0
                            ? `${C.red}08`
                            : 'transparent'
                        const assignedStyle = !t.assigned_to
                          ? {
                          ...
                          <div key={t.id}>
                            ...
                            >
                              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                ...
                                <a
                                  href={`https://redmine.thinkingcode.com/issues/${t.redmine_id || t.id}`}
                                  target="_blank"
                                  rel="noreferrer"
                                  style={{
                                    fontSize: 11,
                              ...
                              <div style={{ fontSize: 11, color: C.dim }}>
                                {fmtDate(t.updated_at || t.last_update)}
                              </div>
                              <div
                                style={{
                                  fontSize: 12,
                              ...
                              >
                                {t.assigned_to ? renderName(t.assigned_to) : 'Unassigned'}
                              </div>
                              <div
                                style={{
                                  fontSize: 11,
...
function People({ people = [], overview = {}, onSelectPerson, onPrepOneOnOne }) {
  ...
  const [teamTimeLogFilters, setTeamTimeLogFilters] = useState({})

  useEffect(() => {
    fetch('/api/pm-pulse/developer-load')
      .then((r) => r.json())
      .then(setDevLoad)
      .catch(() => {})
  }, [])
  ...
  function healthScores(ld, tl) {
    ...
    return {
      bars: [
        ...
        Math.round(overdueScore),
        Math.round(engageScore),
        Math.round(ageingScore),
      ],
      composite,
    }
  }
  ...
  const teamGroups =
    selectedTeam === 'All'
      ? groupBy(
          people.filter((p) => EXPECTED_TIME_TEAMS.includes(p.team)),
          'team',
        )
      : { [selectedTeam]: people.filter((p) => p.team === selectedTeam) }

  const loadRows = (devLoad?.load || []).filter((r) =>
    selectedTeam === 'All' ? EXPECTED_TIME_TEAMS.includes(r.team) : r.team === selectedTeam,
  )
  const timeLogRowsByTeam = timeLogData.filter((r) =>
    selectedTeam === 'All' ? EXPECTED_TIME_TEAMS.includes(r.team) : r.team === selectedTeam,
  )
  const timeLogRows =
    timeLogFilter === 'All'
  ...
  const PEOPLE_TABS = ['Developer Details', 'Time Log', 'Members']
  const TL_FILTERS = [
    'All',
    'Recently Logged',
    'No Log 3+ Days',
  ...
  const loadByTeam = {}
  ;(devLoad?.load || []).forEach((r) => {
    ;(loadByTeam[r.team] = loadByTeam[r.team] || []).push(r)
  })
  const timeLogByTeam = {}
  ;(devLoad?.timeLog || []).forEach((r) => {
    ;(timeLogByTeam[r.team] = timeLogByTeam[r.team] || []).push(r)
  })
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 10 }}>
        {teamStats.map((t) => {
          ...
          const complianceColor =
            t.compliance >= 80 ? C.green : t.compliance >= 50 ? C.amber : C.red
          return (
            <Card
              key={t.team}
            ...
            >
              ...
              >
                <span style={{ fontSize: 14, fontWeight: 700, color: C.white }}>{t.team}</span>
                {t.onLeave > 0 && (
                  <span
                    style={{
                      fontSize: 10,
              ...
              >
                ...
                >
                  {t.hoursToday.toFixed(1)}h
                </span>
                <span style={{ fontSize: 11, color: complianceColor, fontWeight: 600 }}>
                  {t.compliance}% logged
                </span>
              </div>
            </Card>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        {TEAM_PILLS.map((b) => (
          <button
            ...
            style={{
              background: selectedTeam === b ? C.blue : 'transparent',
              border: `1px solid ${selectedTeam === b ? C.blue : C.border}`,
              color: selectedTeam === b ? C.white : C.dim,
              borderRadius: 20,
              padding: '5px 14px',
              fontSize: 12,
              fontWeight: 600,
      ...
      {/* ── TEAM ACCORDIONS (tabbed: Developer Details / Time Log / Members) ── */}
      {Object.entries(teamGroups).map(([team, members]) => {
        const onL = members.filter((m) => m.leave).length
        const activeTab = teamTabs[team] || 'Developer Details'
        const setTab = (tab) => setTeamTabs((prev) => ({ ...prev, [team]: tab }))
        const teamLoadRows = loadByTeam[team] || []
        const teamTimeLogAll = timeLogByTeam[team] || []
        const tlFilter = teamTimeLogFilters[team] || 'All'
        const teamTimeLogRows =
          tlFilter === 'All'
            ? teamTimeLogAll
            : tlFilter === 'Last Logged'
              ? [...teamTimeLogAll].sort((a, b) =>
                  (b.last_log_date || '').localeCompare(a.last_log_date || ''),
                )
              : teamTimeLogAll.filter((r) => r.logging_status === tlFilter)
        ...
        return (
          ...
          >
              ...
              {PEOPLE_TABS.map((tab) => (
                <button
                  ...
                  style={{
                    ...
                    color: activeTab === tab ? C.white : C.dim,
                    borderRadius: 20,
                    padding: '4px 14px',
                    fontSize: 11,
                    fontWeight: 600,
            ...
            {/* Tab: Developer Details (default) */}
            {activeTab === 'Developer Details' &&
              (devLoad ? (
                teamLoadRows.length === 0 ? (
                  <div style={{ padding: '12px 0', color: C.dimmer, fontSize: 12 }}>
                    No developer data available for {team}.
                  </div>
                ) : (
                  <div style={{ overflowX: 'auto' }}>
                    <div style={{ minWidth: 900 }}>
                      <div
                        style={{
                          display: 'grid',
                          gridTemplateColumns:
                            '2fr 1fr 0.7fr 0.7fr 0.7fr 0.7fr 0.7fr 0.7fr 0.7fr 0.7fr 0.7fr 0.7fr 0.7fr 2fr 1fr 1fr',
                          gap: 10,
                          padding: '8px 10px',
                          marginBottom: 2,
                        }}
                        ...
                        return (
                          <div
                            ...
                            style={{
                              ...
                              borderRadius: 8,
                              borderBottom:
                                i < teamLoadRows.length - 1 ? `1px solid ${C.border}` : 'none',
                              alignItems: 'center',
                              ...(isOverloaded ? { background: `${C.amber}10` } : {}),
                            }}
                            onMouseEnter={(e) => (e.currentTarget.style.background = '#0c1a2e')}
                            onMouseLeave={(e) =>
                              (e.currentTarget.style.background = isOverloaded
                                ? `${C.amber}10`
                                : 'transparent')
                            }
                          >
                            ...
                            <span style={{ fontSize: 12, color: C.dim }}>{r.team}</span>
                            <span
                              style={{
                                fontFamily: "'Barlow Condensed',sans-serif",
                                fontSize: 18,
                                fontWeight: 700,
                                color: isOverloaded ? C.amber : C.blueLight,
                              }}
                            >
                              {r.total}
                            </span>
                            <span style={{ fontSize: 12, color: C.dimmer }}>{r.new_count}</span>
                            <span style={{ fontSize: 12, color: C.blueLight }}>
                              {r.in_progress_count}
                            </span>
                            <span style={{ fontSize: 12, color: C.amber }}>{r.reopen_count}</span>
                            <span
                              style={{
                                fontSize: 12,
                                color: r.overdue ? C.red : C.dimmer,
                                fontWeight: r.overdue ? 700 : 400,
                              }}
                            >
                              {r.overdue}
                            </span>
                            <span style={{ fontSize: 12, color: r.due_soon ? C.amber : C.dimmer }}>
                              {r.due_soon}
                            </span>
                            <span
                              style={{ fontSize: 12, color: r.high_priority ? C.red : C.dimmer }}
                            >
                              {r.high_priority}
                            </span>
                            <span
                              style={{
                                ...
                                fontSize: 14,
                                fontWeight: 700,
                                color: r.avg_age_days > 10 ? C.amber : C.dim,
                              }}
                            >
                              {r.avg_age_days || 0}
                            </span>
                            <span
                              style={{
                                ...
                                fontSize: 14,
                                fontWeight: 700,
                                color: r.max_age_days > 15 ? C.red : C.dim,
                              }}
                            >
                              {r.max_age_days || 0}
                            </span>
                            <span
                              style={{ fontSize: 12, color: r.tickets_7plus ? C.amber : C.dimmer }}
                            >
                              {r.tickets_7plus || 0}
                            </span>
                            <span
                              style={{ fontSize: 12, color: r.tickets_15plus ? C.red : C.dimmer }}
                            >
                              {r.tickets_15plus || 0}
                            </span>
                            <span
                            ...
                            <span style={{ fontSize: 12, color: C.dim }}>
                              {fmtDate(r.oldest_ticket_date)}
                            </span>
                            <span style={{ fontSize: 12, color: C.dim }}>
                              {fmtDate(r.latest_update)}
                            </span>
                          </div>
                        )
                      })}
            ...
            {activeTab === 'Time Log' &&
              (devLoad ? (
                <>
                  ...
                  >
                    {TL_FILTERS.map((s) => (
                      <button
                        key={s}
                        onClick={() => setTeamTimeLogFilters((prev) => ({ ...prev, [team]: s }))}
                        style={{
                          background: tlFilter === s ? C.blue : 'transparent',
                          border: `1px solid ${tlFilter === s ? C.blue : C.border}`,
                          color: tlFilter === s ? C.white : C.dim,
                          borderRadius: 20,
                          padding: '4px 12px',
                          fontSize: 11,
                          fontWeight: 600,
                  ...
                  {teamTimeLogRows.length === 0 ? (
                    <div style={{ padding: '12px 0', color: C.dimmer, fontSize: 12 }}>
                      No time log data for {team}.
                    </div>
                  ) : (
                    <div style={{ overflowX: 'auto' }}>
                      <div style={{ minWidth: 700 }}>
                        <div
                          style={{
                            display: 'grid',
                            gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr 1.2fr 0.8fr 1.5fr',
                            gap: 10,
                            padding: '8px 10px',
                            marginBottom: 2,
                          }}
                        ...
                        {teamTimeLogRows.map((r, i) => {
                          ...
                          const isLastLoggedActive = tlFilter === 'Last Logged'
                          const logAge = r.last_log_date
                            ? Math.floor((Date.now() - new Date(r.last_log_date)) / 86400000)
                            : 999
                          const dateColor = logAge <= 7 ? C.green : logAge <= 14 ? C.amber : C.red
                          return (
                            <div
                              key={r.id || i}
                              style={{
                                ...
                                borderRadius: 8,
                                borderBottom:
                                  i < teamTimeLogRows.length - 1 ? `1px solid ${C.border}` : 'none',
                                alignItems: 'center',
                              }}
                              onMouseEnter={(e) => (e.currentTarget.style.background = '#0c1a2e')}
                              onMouseLeave={(e) =>
                            ...
                            >
                              ...
                              >
                                {(r.hours_yesterday || 0).toFixed(1)}h
                              </span>
                              <span
                                style={{
                                  fontFamily: "'Barlow Condensed',sans-serif",
                              ...
                              >
                                {(r.hours_last_7days || 0).toFixed(1)}h
                              </span>
                              <span
                                style={{
                                  fontSize: 12,
                                  color: isLastLoggedActive ? dateColor : C.dim,
                                  fontWeight: isLastLoggedActive && dateColor !== C.red ? 700 : 400,
                                }}
                              >
                                {fmtDate(r.last_log_date)}
                              </span>
                              <span
                                style={{
                                  fontFamily: "'Barlow Condensed',sans-serif",
                                  fontSize: 16,
                                  fontWeight: 700,
                                  color: isNoLog || isLate ? statusColor : C.dim,
                                }}
                              >
                                {r.days_since_last_log ?? '—'}
                              </span>
                              <span
                                style={{
                                  fontSize: 11,
                ...
                <div style={{ padding: '12px 0', color: C.dimmer, fontSize: 12 }}>
                  Loading time log data...
                </div>
              ))}

            {/* Tab: Members (health scores, load bars, status, 1-on-1) */}
            {activeTab === 'Members' && (
              <div style={{ overflowX: 'auto' }}>
                <div style={{ minWidth: 600 }}>
                  <div
                    style={{
                      display: 'grid',
                      gridTemplateColumns: '2fr 56px 2fr 70px 90px',
                      gap: 16,
                      padding: '8px 10px',
                      marginBottom: 4,
                    }}
                  >
                    {['Name', 'Health', 'Load · Overdue · Engage · Age', 'Status', ''].map((h) => (
                      <div
                        key={h}
                        style={{
                          fontSize: 11,
                          ...
                          >
                            ...
                            >
                              ...
                              <div>
                                ...
                                <div style={{ fontSize: 11, color: C.dimmer, marginTop: 1 }}>
                                  {m.team}
                                </div>
                              </div>
                            </div>
                            ...
                            <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                              {(h ? h.bars : [0, 0, 0, 0]).map((score, bi) => {
                                const barColor =
                                  score >= 70 ? BAR_COLORS[bi] : score >= 40 ? C.amber : C.red
                                return (
                                  <div
                                    key={bi}
                                    style={{
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                              {m.leave ? (
                                <>
                                  <Dot color={C.amber} pulse />
                                  <span style={{ fontSize: 12, color: C.amber }}>{m.leave}</span>
                                </>
                              ) : (
                                <>
                                  <Dot color={C.green} />
                                  <span style={{ fontSize: 12, color: C.dim }}>Active</span>
                                </>
                              )}
                            </div>
                            ...
                            <button
                              ...
                              onMouseEnter={(e) => {
                                e.currentTarget.style.background = `${C.blue}33`
                              }}
                              onMouseLeave={(e) => {
      ...
      {/* ── DEVELOPER WORKLOAD SECTIONS (standalone, visible only for "All" view) ── */}
      {devLoad && selectedTeam === 'All' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 8 }}>
          ...
          <AnomalySection
            ...
            rows={loadRows.map((r) => {
              const isOverloaded = r.total > 8
              const rowStyle = isOverloaded
                ? { background: `${C.amber}15`, borderRadius: 6, padding: '2px 6px' }
                : {}
              return [
                <span style={{ fontSize: 13, color: C.white, fontWeight: 600 }}>{r.name}</span>,
                <span style={{ fontSize: 12, color: C.dim }}>{r.team}</span>,
                <span
                  style={{
                    fontFamily: "'Barlow Condensed',sans-serif",
                ...
                <span style={{ fontSize: 12, color: C.dimmer }}>{r.new_count}</span>,
                <span style={{ fontSize: 12, color: C.blueLight }}>{r.in_progress_count}</span>,
                <span style={{ fontSize: 12, color: C.amber }}>{r.reopen_count}</span>,
                <span
                  style={{
                    fontSize: 12,
                ...
                <span style={{ fontSize: 12, color: r.high_priority ? C.red : C.dimmer }}>
                  {r.high_priority}
                </span>,
                <span
                  style={{
                ...
                <span style={{ fontSize: 12, color: r.tickets_7plus ? C.amber : C.dimmer }}>
                  {r.tickets_7plus || 0}
                </span>,
                <span style={{ fontSize: 12, color: r.tickets_15plus ? C.red : C.dimmer }}>
                  {r.tickets_15plus || 0}
                </span>,
                <span
                  style={{
                ...
                <span style={{ fontSize: 12, color: C.dim }}>{fmtDate(r.oldest_ticket_date)}</span>,
                <span style={{ fontSize: 12, color: C.dim }}>{fmtDate(r.latest_update)}</span>,
              ]
            })}
            columns={[
              'Developer',
          ...
          >
            ...
            {STATUS_PILLS.map((s) => (
              <button
                key={s}
                onClick={() => setTimeLogFilter(s)}
                style={{
                  background: timeLogFilter === s ? C.blue : 'transparent',
                  border: `1px solid ${timeLogFilter === s ? C.blue : C.border}`,
                  color: timeLogFilter === s ? C.white : C.dim,
                  borderRadius: 20,
                  padding: '4px 12px',
                  fontSize: 11,
                  fontWeight: 600,
          ...
          <AnomalySection
            ...
            rows={timeLogRows.map((r) => {
              ...
              return [
                ...
                <span style={{ fontSize: 12, color: C.dim }}>{fmtDate(r.last_log_date)}</span>,
                <span
                  style={{
                    fontFamily: "'Barlow Condensed',sans-serif",
                    fontSize: 16,
...
const NAV_ICONS = {
  ...
  Team: (
    ...
    >
      ...
      <path d="M21 18c0-2.5-1.5-4.5-3.5-5.2" />
    </svg>
  ),
  Time: (
    <svg
        ...
        <>
            ...
            >
              ...
              {pendingCount > 0 && (
                <div style={{ marginTop: 8, color: C.amber }}>
                  ⚠ {pendingCount} pending update{pendingCount !== 1 ? 's' : ''} in queue
                </div>
              )}
          ...
          <div style={{ fontSize: 12, color: C.dim, marginBottom: 10 }}>
            ...
            <span style={{ color: C.dimmer }}>
              (leave blank to use current: <b style={{ color: C.white }}>{defaultUrl}</b>)
            </span>
          </div>
          ...
          <div style={{ marginTop: 14, fontSize: 11, color: C.dimmer }}>
            This is a <b>one-time setup</b>. Only re-register if the domain changes. Adding new bot
            users only requires updating their Telegram ID in Dashboard Users above.
          </div>
        </>
      )}
    </Card>
...
function UserDemandsCard() {
  ...
  const STATUS_COLORS = {
    unreviewed: C.dimmer,
    planned: C.amber,
    building: C.blueLight,
    done: C.green,
  }
  const FREQ_COLOR = (f) => (f >= 5 ? C.red : f >= 3 ? C.amber : C.dim)

  useEffect(() => {
    fetch('/api/admin/unknown-queries')
      .then((r) => r.json())
      .then((d) => {
  ...
  return (
    <Card style={{ padding: 24 }}>
      ...
      {queries.length === 0 ? (
        <div style={{ fontSize: 12, color: C.dimmer, textAlign: 'center', padding: '20px 0' }}>
          No unknown queries yet — the bot logs here when it can't answer something.
        </div>
      ) : (
          ...
          {queries.map((q) => (
            ...
            >
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 13, color: C.white, marginBottom: 4, lineHeight: 1.4 }}>
                  {q.query_text}
                </div>
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
                  {q.asked_by && (
                    <span style={{ fontSize: 11, color: C.dimmer }}>{q.asked_by}</span>
                  )}
                  {q.user_role && (
                    <span
                      style={{
                  ...
                  <span style={{ fontSize: 11, color: FREQ_COLOR(q.frequency), fontWeight: 700 }}>
                    {q.frequency}×
                  </span>
                  {q.suggested_alternative && (
                    <span style={{ fontSize: 11, color: C.amber, fontStyle: 'italic' }}>
                      Suggested: {q.suggested_alternative}
                    </span>
                  )}
                </div>
              </div>
              <select
                ...
                style={{
                  background: C.card,
                  border: `1px solid ${STATUS_COLORS[q.status]}44`,
              ...
              >
                {['unreviewed', 'planned', 'building', 'done'].map((s) => (
                  <option key={s} value={s} style={{ color: C.white, background: C.card }}>
                    {s}
                  </option>
                ))}
              </select>
...
function DashboardUsersCard() {
  ...
  function startEdit(u) {
    setEditingId(u.id)
    const bp =
      typeof u.behavior_profile === 'string'
        ? (() => {
            try {
              return JSON.parse(u.behavior_profile)
            } catch {
              return {}
            }
          })()
  ...
  async function save() {
    ...
    if (editingId === 'new') {
      if (!form.username?.trim() || !form.password?.trim() || !form.display_name?.trim()) {
        alert('Username, password and display name are required')
        setSaving(false)
        return
      }
      const res = await fetch('/api/admin/dashboard-users', {
        ...
        body: JSON.stringify({
          ...
          linked_redmine_user_id: form.linked_redmine_user_id || null,
        }),
      })
        ...
        {users.map((u) => (
          <div
            ...
            style={{
              ...
              padding: '10px 14px',
              background: C.bg,
              border: `1px solid ${u.active ? C.border : `${C.border}60`}`,
              borderRadius: 8,
      ...
      {editingId && (
        ...
        >
          <div style={{ fontSize: 12, fontWeight: 600, color: C.white, marginBottom: 14 }}>
            {editingId === 'new'
              ? 'New Dashboard User'
              : `Edit — ${users.find((u) => u.id === editingId)?.display_name}`}
          </div>
          ...
          {editingId === 'new' && (
            <div style={{ marginBottom: 14 }}>
              <div style={lbl}>
                ...
                <span style={{ color: C.dimmer, fontWeight: 400 }}>(auto-fills name & team)</span>
              </div>
              <div style={{ position: 'relative' }}>
                ...
                {search && (
                  <button
                    onClick={() => {
                      setSearch('')
                      setForm((f) => ({ ...f, display_name: '', email: '', team: '' }))
                    }}
                    style={{
                      position: 'absolute',
                      right: 8,
                    ...
                    {filteredRU.map((ru) => (
                      ...
                      >
                        ...
                        <span style={{ color: C.dimmer, fontSize: 11 }}>
                          {ru.email || 'no email'} {ru.team ? `· ${ru.team}` : ''}
                        </span>
                      </div>
                    ))}
                    <div
              ...
              {manualEmail && (
                <div style={{ marginTop: 8 }}>
                  <div style={lbl}>
                    Email <span style={{ color: C.dimmer, fontWeight: 400 }}>(not in Redmine)</span>
                  </div>
                  <input
                    value={form.email || ''}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <div>
              ...
              <input
                ...
                placeholder="QA / Java / DB …"
                style={inp}
              />
            </div>
            <div>
              <div style={lbl}>
                Telegram ID <span style={{ color: C.dimmer, fontWeight: 400 }}>(numeric)</span>
              </div>
              <input
                value={form.telegram_id || ''}
                onChange={(e) => setForm((f) => ({ ...f, telegram_id: e.target.value }))}
                placeholder="e.g. 8600897389"
                style={inp}
              />
            </div>
          </div>
          ...
          <div style={{ fontSize: 11, color: C.dimmer, marginTop: 10 }}>
            Telegram ID: ask user to message{' '}
            <span style={{ color: C.blueLight }}>@userinfobot</span> — or they'll see it when they
            first message the bot.
          </div>
            ...
            >
              ...
              >
                <div>
                  ...
                  <select
                    value={form.response_style || 'adaptive'}
                    onChange={(e) => setForm((f) => ({ ...f, response_style: e.target.value }))}
                    style={{ ...inp, cursor: 'pointer' }}
                  >
                ...
                <div>
                  ...
                  <select
                    value={form.morning_briefing || 'none'}
                    onChange={(e) => setForm((f) => ({ ...f, morning_briefing: e.target.value }))}
                    style={{ ...inp, cursor: 'pointer' }}
                  >
              ...
              <div>
                <div style={lbl}>
                  ...
                  <span style={{ color: C.dimmer, fontWeight: 400 }}>
                    (bot will proactively surface these)
                  </span>
                </div>
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          color: active ? C.blueLight : C.dim,
                          borderRadius: 6,
                          padding: '4px 10px',
                          fontSize: 11,
...
function AIConfigCard() {
  ...
  function startNew() {
    ...
    setForm({
      provider: 'openrouter',
      api_key: '',
      base_url: PROVIDER_URL.openrouter,
      default_model: '',
    ...
    <Card style={{ padding: 24 }}>
      ...
      {(!editingId || editingId === 'new') && disp && (
        <div
          style={{
            background: C.bg,
            border: `1px solid ${pColor(disp.provider)}44`,
            borderRadius: 10,
            padding: 16,
            marginBottom: editingId === 'new' ? 16 : 0,
          }}
        >
          <div
            style={{
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div
                style={{
                  background: `${pColor(disp.provider)}22`,
                  border: `1px solid ${pColor(disp.provider)}66`,
                  borderRadius: 6,
                  padding: '3px 10px',
                  fontSize: 11,
                  fontWeight: 700,
                  color: pColor(disp.provider),
                  textTransform: 'uppercase',
                  letterSpacing: '0.04em',
                }}
              >
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px 28px' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: C.white, fontFamily: 'monospace' }}>
                {disp.api_key_preview || '••••••••'}
              </div>
            </div>
            <div>
              <div style={lbl}>Base URL</div>
              <div style={{ fontSize: 11, color: C.dim, wordBreak: 'break-all' }}>
                {disp.base_url || '—'}
              </div>
            </div>
            <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              ...
              <select
                ...
                onChange={(e) =>
                  setForm((f) => ({
                    ...
                    base_url: PROVIDER_URL[e.target.value] || f.base_url,
                  }))
                }
                style={{ ...inp, cursor: 'pointer' }}
              >
            ...
            <div>
              <div style={lbl}>
                API Key{' '}
                {editingId !== 'new' && (
                  <span style={{ color: C.dimmer, fontWeight: 400 }}>(blank = keep current)</span>
                )}
              </div>
              <input
                type="password"
                ...
                placeholder={editingId !== 'new' ? '(unchanged)' : 'sk-or-v1-...'}
                style={inp}
              />
            </div>
...
function Admin() {
  ...
  const managers = users.filter(
    (u) => u.role === 'Manager' || projects.some((p) => p.manager_id === u.id),
  )
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 30 }}>
      ...
      <Card style={{ padding: 24 }}>
        ...
        <div style={{ marginTop: 10, fontSize: 12, color: C.dimmer }}>
          Unmapped employees: {unmapped.length}
        </div>
          ...
          {TEAMS.map((team) => {
            ...
            return (
              ...
              >
                  ...
                  {members.map((m) => (
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        <span style={{ fontSize: 12, color: m.is_team_lead ? C.amber : C.dim }}>
                          {m.name}
                        </span>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        <button
                          onClick={() => updateUser(m.id, { is_team_lead: !m.is_team_lead })}
                          style={{
                            background: 'none',
                            border: 'none',
                            color: C.blueLight,
                        ...
                        >
                          {m.is_team_lead ? 'Revoke Lead' : 'Make Lead'}
                        </button>
                        <button
                          onClick={() => updateUser(m.id, { team: null, is_team_lead: false })}
                          style={{
                            background: 'none',
                            border: 'none',
                            color: C.dimmer,
                ...
                <select
                  value=""
                  onChange={(e) => updateUser(parseInt(e.target.value, 10), { team })}
                  style={{
                    width: '100%',
                    padding: '8px 12px',
                    borderRadius: 6,
      ...
      <Card style={{ padding: 24 }}>
        ...
        >
          ...
          <select
            value=""
            onChange={(e) => updateUser(parseInt(e.target.value, 10), { role: 'Manager' })}
            style={{
              padding: '8px 16px',
              borderRadius: 8,
              background: C.blue,
          ...
          {managers.map((mgr) => {
            ...
            return (
              ...
              >
                ...
                >
                  <div style={{ fontSize: 13, fontWeight: 700, color: C.blueLight }}>
                    {mgr.name}
                  </div>
                  <button
                    onClick={() => {
                      updateUser(mgr.id, { role: 'Member' })
                      mProps.forEach((p) => updateProject(p.id, null))
                    }}
                    style={{
                      background: 'none',
                      border: `1px solid ${C.borderHi}`,
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 14 }}>
                  ...
                  {mProps.map((p) => (
                    ...
                    >
                      ...
                      <button
                        onClick={() => updateProject(p.id, null)}
                        style={{
                          background: 'none',
                          border: 'none',
                          color: C.dimmer,
                ...
                <select
                  value=""
                  onChange={(e) => updateProject(parseInt(e.target.value, 10), mgr.id)}
                  style={{
                    width: '100%',
                    padding: '8px 12px',
                    borderRadius: 6,
    ...
    >
      ...
      >
            ...
            return (
              <button
                ...
                style={{
                  ...
                  border: 'none',
                  cursor: 'pointer',
                  background: active ? 'rgba(26,110,245,0.15)' : 'transparent',
        ...
        <div style={{ padding: '16px 12px', borderTop: `1px solid ${C.border}` }}>
          ...
          <div style={{ fontSize: 11, color: C.dimmer, marginBottom: 8, paddingLeft: 4 }}>
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            <button
              ...
              disabled={syncing}
              style={{
                background: syncing ? 'transparent' : C.card,
                border: `1px solid ${C.borderHi}`,
                color: syncing ? C.dimmer : C.white,
                borderRadius: 8,
                padding: '8px 16px',
                fontSize: 12,
                fontWeight: 600,
              ...
              {tickets.filter((t) => t.overdue).length > 0 && (
                ...
                >
                  ...
                  <span style={{ fontSize: 12, color: C.red, fontWeight: 600 }}>
                    {tickets.filter((t) => t.overdue).length} overdue
                  </span>
                </div>
              )}
              <Avatar
