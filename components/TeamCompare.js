...
export default function TeamCompare({ teams = [], people = [], tickets = [], timeLogs = [] }) {
  const teamNames = [...new Set([...people.map((p) => p.team), ...teams.map((t) => t.team)])]
    .filter(Boolean)
    .sort()
  ...
  function getTeamStats(teamName) {
    ...
    const avgHours =
      teamPeople.length > 0 ? Math.round((totalHours / teamPeople.length) * 10) / 10 : 0
    ...
    return {
      ...
      avgTicketsPerPerson:
        workloadEntry?.avg_tickets_per_person ||
        (teamPeople.length > 0 ? Math.round(teamTickets.length / teamPeople.length) : 0),
    }
  }
  ...
  return (
    ...
    >
      ...
      <CompareBar
        label="Open Tickets"
        valA={statsA.openTickets}
        valB={statsB.openTickets}
        colorA={C.blueLight}
        colorB={C.green}
      />
      <CompareBar
      ...
      <CompareBar
        ...
        valA={statsA.avgHours}
        valB={statsB.avgHours}
        colorA={C.blueLight}
        colorB={C.green}
      />
      <CompareBar
        label="Health Score"
        valA={statsA.healthScore}
        valB={statsB.healthScore}
        maxVal={100}
        colorA={C.blueLight}
        colorB={C.green}
      />
