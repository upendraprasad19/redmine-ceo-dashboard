...
const STATUS_MAP = {
  ...
  closed: ['Closed', 'Resolved', 'Verified', 'Rejected'],
}
  ...
  try {
    switch (toolName) {
      ...
      case 'get_tickets': {
        ...
        let _createdSinceDate = null
        if (created_since) {
          if (created_since === 'this_week') _createdSinceDate = "date_trunc('week', CURRENT_DATE)"
          else if (created_since === 'this_month')
            _createdSinceDate = "date_trunc('month', CURRENT_DATE)"
          else if (created_since === 'this_quarter')
            _createdSinceDate = "date_trunc('quarter', CURRENT_DATE)"
          else _createdSinceDate = `'${created_since}'` // ISO date string
        }
        ...
        if (status === 'overdue') {
          ...
          const excludeClosed = !(!status || status === 'all')
        ...
        const ticketsWithLinks = (rows || []).map((t) => ({
          ...t,
          ticket_link: t.redmine_id
            ? `[TK-${t.redmine_id}](${REDMINE_URL}/issues/${t.redmine_id})`
            : t.ticket_id,
        }))

        const CLOSED_STATUSES = new Set(['Closed', 'Resolved', 'Verified', 'Rejected'])
        const now = new Date()
        const rowsWithFlags = ticketsWithLinks.map((r) => {
        ...
        return JSON.stringify({
          tickets: rowsWithFlags,
          count: rowsWithFlags.length,
          filters_applied: {
            status,
            priority,
            assignee_name,
      ...
      case 'get_person_summary': {
        ...
        if (users.length === 0) {
          return JSON.stringify({
            error: `No person found matching "${person_name}"${teamFilter ? ' in your team' : ''}`,
          })
        }
      ...
      case 'get_capacity_status': {
        ...
        for (const r of rows) {
          ...
          teamSummary[r.team].total_tickets += parseInt(r.active_tickets || 0, 10)
        }
        for (const t of Object.keys(teamSummary)) {
          teamSummary[t].avg_workload = Math.round(
            teamSummary[t].total_workload / teamSummary[t].members,
          )
        }
      ...
      case 'get_performance_report': {
        ...
        for (const r of rows) {
          if (!teamAvgs[r.team]) {
            teamAvgs[r.team] = { total: 0, count: 0 }
          }
      ...
      case 'prepare_one_on_one': {
        ...
        const [tickets, hours, overdue, blockedTickets, recentActivity, perfData, leaveData] =
          await Promise.all([
            sql`
        ...
        const summary = {
          ...
          recent_tickets: tickets
            .slice(0, 5)
            .map((t) => `${t.title} [${t.status}] ${t.project_name || ''}`),
          recent_activity: recentActivity.map((a) => ({
            title: a.title,
            note: (a.notes || '').substring(0, 100),
            date: a.created_at,
          })),
        }
      ...
      case 'get_velocity_prediction': {
        ...
        const closedLast4Weeks = parseInt(closedRecent[0]?.count || 0, 10)
        const openTickets = parseInt(openCount[0]?.count || 0, 10)
        const total = parseInt(totalCount[0]?.count || 0, 10)
        const weeklyVelocity = closedLast4Weeks / 4

        // Calculate predicted completion
        const weeksToComplete = weeklyVelocity > 0 ? Math.ceil(openTickets / weeklyVelocity) : null
        let predictedDate = null
        let willMissDeadline = null

        if (weeksToComplete !== null) {
          const now = new Date()
          predictedDate = new Date(now.getTime() + weeksToComplete * 7 * 24 * 60 * 60 * 1000)
            .toISOString()
            .split('T')[0]
          if (project.deadline) {
      ...
      case 'get_trend_analysis': {
        ...
        if (metric === 'velocity' || metric === 'all') {
          ...
          `
          results.velocity = velocity.map((r) => ({
            week: r.week_start,
        ...
        if (metric === 'compliance' || metric === 'all') {
          ...
          results.compliance = compliance.map((r) => ({
            week: r.week_start,
            logged: parseInt(r.logged_count, 10),
            total: parseInt(r.total_users, 10),
            pct: Math.round(
              (parseInt(r.logged_count, 10) / Math.max(parseInt(r.total_users, 10), 1)) * 100,
            ),
          }))
        }
        ...
        if (metric === 'creation_vs_closure' || metric === 'all') {
          const creation = await sql`
            ...
            GROUP BY week_start ORDER BY week_start ASC
          `
          results.creation_rate = creation.map((r) => ({
            week: r.week_start,
      ...
      case 'set_reminder': {
        ...
        try {
          ...
          if (lower.includes('tomorrow')) {
            ...
            const h = parseInt(lower.match(/in\s+(\d+)\s+hour/)[1], 10)
            remindDate = new Date(Date.now() + h * 3600000)
          } else if (lower.match(/in\s+(\d+)\s+min/)) {
            const m = parseInt(lower.match(/in\s+(\d+)\s+min/)[1], 10)
            remindDate = new Date(Date.now() + m * 60000)
          } else {
            remindDate = new Date(remind_at)
          }
          if (Number.isNaN(remindDate.getTime())) throw new Error('invalid date')
        } catch (_e) {
          return JSON.stringify({
            error: 'Could not parse time. Try "tomorrow 10am" or "in 2 hours".',
          })
        }

        const userRow =
          await sql`SELECT telegram_id FROM dashboard_users WHERE id = ${currentUser.id} LIMIT 1`
        ...
        const formatted = remindDate.toLocaleString('en-IN', {
      ...
      case 'propose_intimation': {
        ...
        if (!target.telegram_id)
          return JSON.stringify({ error: `${target.display_name} hasn't registered Telegram yet.` })
        if (!target.consent_given_at)
          return JSON.stringify({
            error: `${target.display_name} hasn't given consent yet. They need to message the bot and reply /agree first.`,
          })
        ...
        if (issueRow.length === 0)
          return JSON.stringify({ error: `No ticket TK-${issue_redmine_id} found.` })
        ...
        return JSON.stringify({
          ...
          next_step:
            'The bot must render a preview card with [Yes, send] / [Cancel] buttons whose callback_data is `int:confirm:<target_user_id>:<issue_id>` and `int:cancel`.',
        })
      }
