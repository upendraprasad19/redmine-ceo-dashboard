...
async function executeToolCall(toolName, args, currentUser) {
  ...
  try {
    switch (toolName) {
      ...
      // ─────────────────────────────────────────────────
      case 'get_tickets': {
        const { status, priority, assignee_name, project_name, due_within_days } = args || {};
        ...
        if (status === 'overdue') {
          ...
          rows = teamFilter
            ? await sql`
                SELECT
                  i.id, 'TK-' || i.redmine_id AS ticket_id, i.title, i.status, i.priority,
                  i.due_date, p.name AS project_name, u.name AS assigned_to, u.team
                FROM issues i
          ...
          rows = teamFilter
            ? await sql`
                ...
                WHERE u.team = ${teamFilter}
                  ...
                  AND (${!excludeClosed || !!statusValues} OR i.status NOT IN ('Closed', 'Resolved'))
                  AND (${!assignee_name} OR u.name ILIKE ${'%' + (assignee_name || '') + '%'})
                  AND (${!project_name} OR p.name ILIKE ${'%' + (project_name || '') + '%'})
                  AND (${!priority} OR i.priority ILIKE ${'%' + (priority || '') + '%'})
        ...
        return JSON.stringify({
          tickets: rows,
          count: rows.length,
          filters_applied: { status, priority, assignee_name, project_name, due_within_days },
        });
      }
        ...
        const enriched = await Promise.all(projects.map(async (proj) => {
          const stats = await sql`
            SELECT
              ...
              COUNT(*) FILTER (WHERE due_date IS NOT NULL AND due_date < CURRENT_DATE AND status NOT IN ('Closed', 'Resolved')) AS overdue_tickets,
