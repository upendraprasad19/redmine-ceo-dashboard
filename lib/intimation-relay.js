...
async function sendIntimation({ thread_id, originator, target, cc = null, issue, note = '' }) {
  ...
  const overdueLine =
    issue.days_overdue && issue.days_overdue > 0 ? ` (overdue ${issue.days_overdue}d)` : ''
  const noteLine = note ? `\n\n_${note}_` : ''

  const receiverText = `Hi ${targetName}, *${originatorName}* is asking about ${ticketLink} '${issue.subject}'${overdueLine}. Can you share status?${noteLine}`

  await tgSend(target.telegram_id, receiverText, receiverButtons(thread_id))

  if (cc?.telegram_id) {
    const ccText = `FYI: *${originatorName}* intimated *${targetName}* about ${ticketLink}. I'll relay the response back here.`
...
async function relayResponse({ thread, originator, cc = null, responseText, buttonLabel = null }) {
  ...
  if (cc?.telegram_id) await tgSend(cc.telegram_id, body)
}
