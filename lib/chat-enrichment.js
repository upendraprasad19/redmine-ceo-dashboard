...
async function classifyOne(content) {
  ...
  try {
    ...
    const entities = {
      tickets: Array.isArray(parsed.entities?.tickets)
        ? parsed.entities.tickets.filter(Number.isInteger)
        : [],
      users: Array.isArray(parsed.entities?.users)
        ? parsed.entities.users.slice(0, 10).map(String)
        : [],
      projects: Array.isArray(parsed.entities?.projects)
        ? parsed.entities.projects.slice(0, 10).map(String)
        : [],
    }
