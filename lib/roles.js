...
function resolveRole(redmineUser, username) {
  const managers = (process.env.MANAGER_USERNAMES || '')
    .split(',')
    .map((s) => s.trim().toLowerCase())
    .filter(Boolean);
  if (username && managers.includes(username.toLowerCase())) return 'manager';
  if (redmineUser?.is_team_lead) return 'team_lead';
  return 'developer';
}

module.exports = { ROLES, checkAccess, scopeData, canSeeUser, canSeeProject, resolveRole };
