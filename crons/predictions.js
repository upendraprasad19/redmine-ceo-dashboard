  ...
  try {
    ...
    for (const project of projects) {
      try {
        ...
        console.error(`[CRON] Velocity Predictions: error for project ${project.name}:`, predErr.message);
        results.errors++;
      }
    }
        ...
        try {
          ...
          if (sendErr.description && sendErr.description.includes("can't parse")) {
            try {
              ...
              console.error(`[CRON] Velocity Predictions: retry failed for ${mgr.display_name}:`, retryErr.message);
              results.errors++;
            }
          } else {
            console.error(`[CRON] Velocity Predictions: failed for ${mgr.display_name}:`, sendErr.message);
            results.errors++;
          }
        }
      }
