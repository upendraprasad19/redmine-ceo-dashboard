...
const APPROVED = [2,3,5,7,14,15,16,17,18,19,20,21,23,29,34,43,44,47,49,50,51,55,56,57,60,61,62,63,65,67,68,69,70,71,72,73,74,75,76];
...
async function fetchAll(endpoint, key, params = '') {
  let results = [], offset = 0, limit = 100, total = 1;
  while (offset < total) {
...
async function runWeeklyReconcile() {
  ...
  let inserted = 0, corrected = 0, projectsScanned = 0;
  ...
  for (const redmineProjectId of APPROVED) {
    ...
    const issues = await fetchAll('/issues', 'issues', `&project_id=${redmineProjectId}&status_id=*&created_on=>=2025-10-01&sort=id:asc`);
  ...
  console.log(`[weekly-reconcile] Done: ${inserted} inserted, ${corrected} corrected, ${projectsScanned} projects in ${duration}ms`);
