...
const APPROVED_PROJECT_IDS = [
  2, 3, 5, 7, 14, 15, 16, 17, 18, 19, 20, 21, 23, 29, 34, 43, 44, 47, 49,
  50, 51, 55, 56, 57, 60, 61, 62, 63, 65, 67, 68, 69, 70, 71, 72, 73, 74, 75, 76,
];
...
async function runHealthCheck() {
  ...
  try {
    const dbIssues = await sql`
      ...
      WHERE redmine_project_id = ANY(${APPROVED_PROJECT_IDS})
    `;
    ...
    for (let page = 0; page < 100; page++) {
      ...
      const approved = (issues || []).filter((i) =>
        APPROVED_PROJECT_IDS.includes(i.project && i.project.id)
      );
    ...
    if (drifted) {
      warn(
        'ISSUE_DRIFT',
        `Issue count differs ${pct.toFixed(2)}% (>${ISSUE_DRIFT_PCT}%)`,
        { dbCount, redmineCount }
      );
    }
  } catch (err) {
  ...
  try {
    const rows = await sql`
      ...
      WHERE (delivery_owner_ids IS NULL OR array_length(delivery_owner_ids, 1) = 0)
        ...
        AND redmine_project_id = ANY(${APPROVED_PROJECT_IDS})
    `;
    const missing = Number(rows[0].c);
    report.checks.deliveryOwnerDrift = { missingOwnerCount: missing, expectedNearZero: true };
    if (missing > 0) {
      warn(
        'DELIVERY_OWNER_DRIFT',
        `${missing} open issues have no resolved Delivery Owner`,
        { missing }
      );
    }
  } catch (err) {
  ...
  try {
    ...
    if (drifted) {
      warn(
        'USER_DRIFT',
        `User count differs ${pct.toFixed(2)}% (>${USER_DRIFT_PCT}%)`,
        { dbCount, redmineCount }
      );
    }
  } catch (err) {
  ...
  try {
    ...
    if (drifted) {
      warn(
        'TIME_DRIFT',
        `Time entry hours differ ${diff.toFixed(2)}h (>${TIME_DRIFT_HOURS}h)`,
        { dbHours, redmineHours }
      );
    }
  } catch (err) {
