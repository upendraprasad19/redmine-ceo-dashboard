      ...
      try {
        ...
        if (user.telegram_id && report.summary) {
          try {
            ...
            await bot.telegram.sendMessage(user.telegram_id, telegramMsg, { parse_mode: 'Markdown' });
            results.telegramSent++;
          } catch (tgErr) {
            // Retry without Markdown on parse error
            if (tgErr.description && tgErr.description.includes("can't parse")) {
              try {
                ...
                console.error(`[CRON] Weekly Reports: Telegram retry failed for ${user.display_name}:`, retryErr.message);
                results.errors++;
              }
            } else {
              console.error(`[CRON] Weekly Reports: Telegram failed for ${user.display_name}:`, tgErr.message);
              results.errors++;
            }
          }
        }
...
${report.summary.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>').replace(/\n/g, '<br>')}
            ...
            console.error(`[CRON] Weekly Reports: email failed for ${user.display_name}:`, emailErr.message);
            results.errors++;
          }
        }
      } catch (userErr) {
