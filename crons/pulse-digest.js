...
async function runPulseDigest() {
  ...
  try {
    ...
    const completed = (completedRows && completedRows[0]) || { completed: 0, contributors: 0 };
    const hours = (hoursRows && hoursRows[0]) || { total_hours: 0, loggers: 0 };
    const newBlockers = (newBlockersRows && newBlockersRows[0] && newBlockersRows[0].count) || 0;
    ...
    for (const user of (recipients || [])) {
      try {
        ...
        if (sendErr.description && sendErr.description.includes("can't parse")) {
          try {
            await bot.telegram.sendMessage(user.telegram_id, message.replace(/\*/g, ''));
            results.sent++;
          } catch (retryErr) {
            console.error(`[CRON] Pulse Digest: retry failed for ${user.display_name}:`, retryErr.message);
            results.errors++;
          }
        } else {
