...
async function buildDeliveryOwnerEnumMap() {
  ...
  if (!res.ok) {
    console.log('  ⚠️  Cannot fetch custom_fields; Delivery Owner will be empty')
    return
  }
  ...
  console.log(
    `  ✓ Delivery Owner enum map: ${deliveryOwnerEnumToNeonId.size}/${pv.length} resolved`,
  )
  pv.forEach((opt) =>
    console.log(
      `    ${opt.value} → ${opt.label} ${deliveryOwnerEnumToNeonId.has(String(opt.value)) ? '✓' : '(no match)'}`,
    ),
  )
}
