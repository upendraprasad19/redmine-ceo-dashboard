...
async function fetchProjectIssues(projectId) {
  ...
  while (offset < totalCount) {
    const data = await redmineFetch(`/issues.json?project_id=${projectId}&status_id=open&limit=${limit}&offset=${offset}`);
...
async function main() {
  try {
    ...
    const dbProjects = await sql`SELECT redmine_id, name FROM projects WHERE redmine_id = ANY(${APPROVED_REDMINE_IDS}::int[])`;
    ...
    for (const proj of dbProjects) {
      ...
      for (let i = 0; i < updates.length; i += chunkSize) {
        ...
        await Promise.all(chunk.map(async (u) => {
          ...
          if (u.assignedTo) {
            try {
              const uRows = await sql`SELECT id, team FROM users WHERE redmine_id = ${u.assignedTo.id} LIMIT 1`;
              if (uRows.length > 0 && uRows[0].team) {
                const iRows = await sql`SELECT id FROM issues WHERE redmine_id = ${u.redmineId} LIMIT 1`;
                if (iRows.length > 0) {
                  await sql`
