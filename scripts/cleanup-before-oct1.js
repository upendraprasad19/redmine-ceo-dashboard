...
const APPROVED_IDS =
  '{2,3,5,7,14,15,16,17,18,19,20,21,23,29,34,43,44,47,49,50,51,55,56,57,60,61,62,63,65,67,68,69,70,71,72,73,74,75,76}'
...
async function cleanup() {
  ...
  try {
    ...
    const badProjectIds =
      await sql`SELECT id FROM projects WHERE redmine_id != ALL(${APPROVED_IDS}::int[])`
    ...
    if (badProjectIdList.length > 0) {
      ...
      const badIssueIds =
        await sql`SELECT id FROM issues WHERE project_id = ANY(${badProjectIdList})`
      ...
      if (badIssueIdList.length > 0) {
        ...
        let result =
          await sql`DELETE FROM issue_team_history WHERE issue_id = ANY(${badIssueIdList})`
