...
async function verify() {
  ...
  tickets.forEach(t => console.log('#' + t.redmine_id + ' | ' + t.status + ' | ' + t.title.slice(0, 55) + ' | Manager: ' + (t.manager || '—') + ' | Team: ' + (t.team || '—')));
  const s = await sql`SELECT COUNT(*) as total, COUNT(i.delivery_owner_ids) as with_do FROM issues i JOIN projects p ON p.id=i.project_id WHERE p.name='iCLAIMS 2.0' AND i.status NOT IN ('Closed', 'Resolved', 'Verified', 'Rejected')`;
