...
const INTENTS = [
  ...
  },
  {
    name: 'hours_week',
    patterns: [
      /hours?\s*this\s*week/i,
      /weekly\s*hours?/i,
      /time\s*this\s*week/i,
      /week.?s?\s*hours?/i,
    ],
  },
  {
 ...
 */
async function handleDevMessage(message, say, _client) {
...
async function handleOverdue(user, say) {
  ...
  const blocks = [
    {
      ...
      text: {
        type: 'plain_text',
        text: `You have ${tickets.length} overdue ticket(s)`,
        emoji: true,
      },
    },
    { type: 'divider' },
  ...
  for (const t of tickets.slice(0, 10)) {
    ...
    const dueStr = new Date(t.due_date).toLocaleDateString('en-US', {
      month: 'short',
      day: 'numeric',
    })
    blocks.push({
...
async function handleHours(user, say, period) {
  ...
  const entryCount = parseInt(data.entry_count || 0, 10)
  ...
  if (period === 'week' && data.days_logged !== undefined) {
    ...
    const daysLogged = parseInt(data.days_logged || 0, 10)
  ...
  if (totalHours === 0) {
    blocks.push({
      ...
      elements: [
        {
          type: 'mrkdwn',
          text: ':warning: _No time logged yet. Remember to update your time entries!_',
        },
      ],
    })
  }
...
async function handleLeave(user, say) {
  ...
  for (const lr of leaveRecords) {
    const startStr = new Date(lr.start_date).toLocaleDateString('en-US', {
      month: 'short',
      day: 'numeric',
    })
    const endStr = new Date(lr.end_date).toLocaleDateString('en-US', {
      month: 'short',
      day: 'numeric',
    })
...
async function handleFullStatus(user, say) {
  ...
  const [activeResult, overdueResult, blockedResult, hoursToday, hoursWeek, leaveResult] =
    await Promise.all([
      sql`
  ...
  const blocked = parseInt(blockedResult[0]?.count || 0, 10)
  ...
  if (overdue > 0 || blocked > 0) {
    ...
    if (overdue > 0)
      alerts.push(
        `:rotating_light: ${overdue} ticket(s) are overdue — type *"overdue"* for details`,
      )
    if (blocked > 0)
      alerts.push(`:no_entry: ${blocked} ticket(s) are blocked — type *"blockers"* for details`)
    blocks.push({
      type: 'context',
  ...
  const blocks = [
    ...
    },
    {
      ...
      elements: [
        {
          type: 'mrkdwn',
          text: "_Just type any of these phrases and I'll pull your data instantly._",
        },
      ],
    },
  ]
