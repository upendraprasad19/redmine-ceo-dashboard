...
async function sendStyleStep(ctx, name) {
  await ctx.reply(
...
async function sendBriefingTimeStep(ctx) {
  await ctx.reply(
    '*What time? _(IST)_*',
    { parse_mode: 'Markdown', reply_markup: briefingTimeKeyboard() }
  );
}
...
async function sendChallengeStep(ctx, name) {
  await ctx.reply(
    ...
    {
      parse_mode: 'Markdown',
      reply_markup: { inline_keyboard: [[{ text: '⏩ Skip', callback_data: 'ob:challenge:skip' }]] },
    }
  );
}
...
async function sendConfirmStep(ctx, collected, userName) {
  ...
  const briefing = collected.morning_briefing === 'off' || !collected.morning_briefing
    ? 'No briefing'
...
async function handleOnboardingCallback(ctx, user) {
  ...
  if (data === 'ob:welcome:skip') {
    await ctx.editMessageText('No problem! You can personalise anytime with /preferences').catch(() => {});
  ...
  if (data.startsWith('ob:focus:')) {
    ...
    } catch (e) { /* message not modified is OK */ }
...
async function handleOnboardingText(ctx, user) {
  ...
  if (!state || state.step !== 'challenge') return false;
