...
async function loadUser(sql, id) {
  const rows =
    await sql`SELECT id, display_name, username, role, team, telegram_id FROM dashboard_users WHERE id = ${id} LIMIT 1`
  return rows[0] || null
}
...
async function handleIntimationCallback(ctx, botUser) {
  ...
  if (!data?.startsWith('int:')) return false
  ...
  try {
    ...
    if (['ack', 'working', 'blocked'].includes(parts[1])) {
      ...
      if (thread.target_id !== botUser.id) {
      ...
      await ctx
        .editMessageText(`✓ ${buttonLabel} — sent to ${originator.display_name}.`)
        .catch(() => {})
      await ctx.answerCbQuery()
      return true
    }
    ...
    if (parts[1] === 'escalate') {
      ...
      if (thread.cc_user_id) {
        ...
        if (tl?.telegram_id) {
          await relayResponse({
            thread: {
              redmine_id: thread.redmine_id,
              target_display_name: thread.target_display_name,
            },
            originator: tl,
            cc: null,
    ...
    if (parts[1] === 'commit_extend') {
      ...
      if (cr?.originator_tg && cr.redmine_id) {
        try {
      ...
      await ctx
        .editMessageText('Noted — the originator has been told you need more time.')
        .catch(() => {})
      await ctx.answerCbQuery()
      return true
    }
