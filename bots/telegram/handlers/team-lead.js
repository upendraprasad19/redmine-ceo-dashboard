...
function formatMemberWorkload(members) {
  ...
  for (const m of members) {
    ...
    lines.push(`  Workload: ${workloadPct}% | Tickets: ${m.active_tickets || 0} | Available: ${m.available_capacity_pct || 0}%`);
  }
...
function formatOneOnOnePrep(data) {
  ...
  if (performance) {
    ...
    lines.push(`  Score: *${performance.overall_score}* ${trendEmoji} (${performance.trend || 'stable'})`);
    lines.push('');
  }
